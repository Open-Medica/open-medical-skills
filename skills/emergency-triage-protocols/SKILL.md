---
name: emergency-triage-protocols
description: >
  AI-assisted emergency department triage tool with evidence-based protocols
  for rapid patient assessment. Helps prioritize cases based on acuity and
  clinical presentation.
---

# Emergency Department Triage Protocols

AI-assisted emergency department triage tool with evidence-based protocols for rapid patient assessment. Helps prioritize cases based on acuity and clinical presentation.

## Category

emergency

## Specialty

emergency-medicine

## Tags

emergency-medicine, triage, acuity-assessment

## Safety Classification

Caution

## Evidence Level

Moderate

## Author

Healthcare MCP Public

## Version

1.0.0

## License

MIT

## Repository

https://github.com/Open-Medica/open-medical-skills/tree/main/skills/emergency-triage-protocols

## Installation

**npx:**
```bash
npx skills add Open-Medica/open-medical-skills --skill emergency-triage-protocols
```

**git:**
```bash
git clone https://github.com/Open-Medica/open-medical-skills.git && cp -r open-medical-skills/skills/emergency-triage-protocols ~/.claude/skills/
```

---

*This skill is part of [Open Medical Skills](https://github.com/Open-Medica/open-medical-skills), a curated marketplace of medical AI skills maintained by physicians for physicians and the healthcare industry.*
